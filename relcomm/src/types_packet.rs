...
use crate::config::CORRUPTION_RATE;
...
impl PacketType {
    ...
    /// Também emula a corrupção de pacotes
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        let type_h = bytes[0];
        let bytes = bytes[1..].to_vec();
        let mut pkt = match type_h {
            Ack::SD => {
    ...
    pub fn validate_message(&self) -> bool {
        self.get_checksum() == Self::checksum(self)
    }

    /// Chama a função de checksum do pacote
    fn checksum(pkt: &Self) -> u32 {
        match pkt {
            PacketType::Ack(pkt) => <Ack as Header>::checksum(pkt),
            PacketType::Send(pkt) => DataPkt::checksum(pkt),
            PacketType::Broadcast(pkt) => DataPkt::checksum(pkt),
            PacketType::AckBrd(pkt) => <AckBrd as Header>::checksum(pkt),
            PacketType::LeaderRequest(pkt) => DataPkt::checksum(pkt),
        }
    }

    /// Altera o checksum do pacote, para simular corrupção
    fn set_checksum(&mut self, value: u32) {
        match self {
            PacketType::Ack(pkt) => pkt.checksum += value,
            PacketType::AckBrd(pkt) => pkt.checksum += value,
            PacketType::Send(pkt) => pkt.header.checksum += value,
            PacketType::Broadcast(pkt) => pkt.header.checksum += value,
            PacketType::LeaderRequest(pkt) => pkt.header.checksum += value,
        }
    }
}
